...
const generateInput = (forms: any[]) => {
  ...
  ${forms.map((form) => `${form.question}: ${form.answer}`).join("\n")}
...
export const MeetResume = ({ forms }: MeetResumeProps) => {
  ...
  const initChatBot = async () => {
    ...
    const assistant = await openai.beta.assistants.create({
      ...
      instructions:
        "Voce tem o trabalho de gerar um resumo do que foi discutido no encontro semanal de uma equipe de desenvolvimento de sotware para internet das coisas.",
      model: "gpt-4o-mini",
    });
